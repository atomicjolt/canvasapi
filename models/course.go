...
type Course struct {
	...
	Permissions                       map[string](interface{}) `json:"permissions" url:"permissions,omitempty"`                                                     // optional: the permissions the user has for the course. returned only for a single course and include[]=permissions.Example: true, true
	...
	BlueprintRestrictions             map[string](interface{}) `json:"blueprint_restrictions" url:"blueprint_restrictions,omitempty"`                               // optional: Set of restrictions applied to all locked course objects.Example: true, true, false, false
	BlueprintRestrictionsByObjectType map[string](interface{}) `json:"blueprint_restrictions_by_object_type" url:"blueprint_restrictions_by_object_type,omitempty"` // optional: Sets of restrictions differentiated by object type applied to locked course objects.Example: {'content'=>true, 'points'=>true}, {'content'=>true}
...
func (t *Course) HasErrors() error {
	...
	if t.DefaultView != "" && !string_utils.Include(s, t.DefaultView) {
		errs = append(errs, fmt.Sprintf("expected 'DefaultView' to be one of %v", s))
	}
	return nil
}

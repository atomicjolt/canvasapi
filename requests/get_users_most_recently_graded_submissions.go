...
type GetUsersMostRecentlyGradedSubmissions struct {
	...
	Query struct {
		Include                  []string `json:"include"`                    //  (Optional) . Must be one of assignment
		OnlyCurrentEnrollments   bool     `json:"only_current_enrollments"`   //  (Optional)
		OnlyPublishedAssignments bool     `json:"only_published_assignments"` //  (Optional)
	} `json:"query"`
}
...
func (t *GetUsersMostRecentlyGradedSubmissions) GetBody() (string, error) {
	return "", nil
}
...
func (t *GetUsersMostRecentlyGradedSubmissions) HasErrors() error {
	...
	for _, v := range t.Query.Include {
		if !string_utils.Include([]string{"assignment"}, v) {

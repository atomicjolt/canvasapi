...
//
// Form Parameters:
// # Form.Assignment.Name (Required) The assignment name.
// # Form.Assignment.Position (Optional) The position of this assignment in the group when displaying
//    assignment lists.
// # Form.Assignment.SubmissionTypes (Optional) . Must be one of online_quiz, none, on_paper, discussion_topic, external_tool, online_upload, online_text_entry, online_url, media_recording, student_annotationList of supported submission types for the assignment.
...
// # Form.Assignment.DueAt (Optional) The day/time the assignment is due. Must be between the lock dates if there are lock dates.
...
// # Form.Assignment.QuizLti (Optional) Whether this assignment should use the Quizzes 2 LTI tool. Sets the
...
// # Form.Assignment.AllowedAttempts (Optional) The number of submission attempts allowed for this assignment. Set to -1 for unlimited attempts.
...
func (t *CreateAssignment) HasErrors() error {
	...
	if t.Form.Assignment.Name == "" {
		errs = append(errs, "'Form.Assignment.Name' is required")
	}
